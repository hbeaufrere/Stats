---
title: "ZESTR: Zoological Education on Statistics - Tutorial on R"
...
Tip #1: To import a dataset with RStudio, save them as "dataname.csv" and import by clicking on environment (top right panel) and then the table icon "Import Dataset", then "from Text(base)".
...
First, could you get the prevalence of each lipid-related disorder in these Quaker parrots?
...
If you have numbers in the table that are below 5, then you are not supposed to use a Chi square test, but instead a Fisher exact test. Look for instance at the numbers of lipomas (do it yourself).
...
Try to change the code above and run a Chi square test to see what happens (hehe).
